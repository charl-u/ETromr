...
class CustomVisionTransformer(VisionTransformer):
    ...
    def forward_features(self, x):
        B, c, h, w = x.shape
        x = self.patch_embed(x) # [B, patch_len, encoder_dim]
        ...
        x = self.norm(x)
        return x # [b, patch_len + 1, encoder_dim]
...
class ViTEncoder(nn.Module):
    def __init__(self, 
        ...
        args['encoder_heads'] = encoder_heads
...
if __name__ == '__main__':
    model = ViTEncoder(max_height=128, 
                       max_width=1024, 
                       channels=1, 
                       patch_size=16, 
                       encoder_dim=512, 
