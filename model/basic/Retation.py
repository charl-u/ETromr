...
def _build_decay_mask(
    ...
    """
    ## TODO: RMT中修改casual mask的位置，使其能够关注到之后的信息
    if type == 'retnet':
        ...
        xmin, xmax = log(1 / 32), log(1 / 512)
        x = torch.linspace(xmin, xmax, steps=num_heads//2, device=device, dtype=dtype)
        offset = x.exp()
        decay_gammas_1 = 1 + offset
        decay_gammas_2 = 1 + offset
        decay_gammas = torch.concat([decay_gammas_1, torch.flip(decay_gammas_2, dims=[0])], dim=-1)
        query_pos = torch.arange(query_length, device=device, dtype=dtype).unsqueeze_(-1)
        key_pos = torch.arange(key_length, device=device, dtype=dtype).unsqueeze_(0)
        distance = torch.abs(query_pos - key_pos)
        distance = rearrange(distance, "n s -> () n s")
        ...
        condition = torch.any(decay_mask > 1, dim=-1)
        update_mask = decay_mask[condition]
        max, _ = torch.max(update_mask, dim=-1)
        update_mask = update_mask / max.unsqueeze(-1)
        decay_mask[condition] = update_mask
        ...
        mask = torch.ones(num_heads, query_length, key_length, device=device, dtype=dtype)
        if casual:
            future_mask = torch.ones_like(mask, dtype=torch.bool).triu_(diagonal=1)
            return mask.masked_fill_(future_mask, 0)
        return mask
    elif type == 'custom2':
        decay_gammas = _build_decay_gammas(num_heads=num_heads//2, device=device, dtype=dtype)
        query_pos = torch.arange(query_length, device=device, dtype=dtype).unsqueeze_(-1)
        key_pos = torch.arange(key_length, device=device, dtype=dtype).unsqueeze_(0)
        distance_1 = torch.abs(query_pos - key_pos)
        distance_2 = query_length -1 - distance_1
        decay_gammas = rearrange(decay_gammas, "h -> h () ()")
        
        distance_1 = rearrange(distance_1, "n s -> () n s")
        decay_mask_1 = decay_gammas**distance_1

        distance_2 = rearrange(distance_2, "n s -> () n s")
        decay_mask_2 = decay_gammas**distance_2

        decay_mask = torch.concat([decay_mask_1, decay_mask_2], dim=0)
        if casual:
            future_mask = torch.ones_like(decay_mask, dtype=torch.bool).triu_(diagonal=1)
            return decay_mask.masked_fill_(future_mask, 0)
        return decay_mask
...
class MultiScaleRetention(nn.Module):
    ...
    def forward_parallel(
        ...
        retention, weights = retention_parallel(q, k, v, need_weights=need_weights, casual=self.casual, mask_type=self.mask_type)
